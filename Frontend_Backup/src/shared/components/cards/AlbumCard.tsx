...
export const AlbumCard = memo(function AlbumCard({ album }: AlbumCardProps) {
  return (
    <Link href={`/album/${album.id}`} className="flex-shrink-0 group cursor-pointer w-48 block">
      <div className="relative rounded-2xl overflow-hidden aspect-square glass-card mb-4 group-hover:scale-[1.04] transition-all duration-300">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <div className="w-12 h-12 bg-white/10 border border-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white">
            <span className="material-symbols-outlined text-2xl">read_more</span>
          </div>
        </div>
      </div>
      ...
      <p className="text-white/50 text-xs truncate">{album.artist.name}</p>
    </Link>
  );
});
